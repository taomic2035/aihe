...
明确 Her 式陪伴的**功能边界与非功能约束**，产出可验收的 Spec。
...
### Phase D（语音、多端与自进化）
- [ ] AC-D1: 语音链路 e2e 延迟 p50 <800ms（级联），支持打断与情感语调，移动端可语音对话（实测录屏）
- [ ] AC-D2: WatchOS thin client 可接收推送、发送文本、同步历史，离线消息云端补偿（实测）
- [ ] AC-D3: 至少 3 个 MCP 助理工具可用（日历/搜索/备忘录），主动关怀按节律触发且可被用户关闭/调节
- [ ] AC-D4: GEPA 自进化跑通 1 轮：合成+真实 session 评估，产出 PR 且通过门闸（测试/大小/语义保真）并经人审合并
- [ ] AC-D5: 合规关口全量通过：AI 披露、自伤检测、危机路由、健康使用提醒均有 e2e 测试
...
|----|---------------------------|---------|----------|------|
| R1 | "有性格" — 稳定人格，长期不漂移 | AC-A1, AC-B2, AC-C2 | SOUL.md + 一致性评估集 + 长期对话测试 | [ ] |
| R2 | "有情感" — 共情、情绪识别与情感化表达 | AC-A1, AC-B2, AC-D1 | 情感分类测试 + TTS prosody 试听 + 人工评分 | [ ] |
| R3 | "永久记忆" — 跨会话、跨端永久记忆，可审计可删 | AC-A2, AC-B3, AC-C3 | 跨会话召回测试 + 删除后不再召回测试 | [ ] |
| R4 | "听" — 语音输入，实时 STT | AC-B4, AC-D1 | 延迟实测 + WER 测试 + 真机录屏 | [ ] |
| R5 | "说" — 语音输出，情感化 TTS/克隆 | AC-B4, AC-D1 | MOS 试听 + 延迟实测 + 克隆对比 | [ ] |
| R6 | "读" — 读文档/读图/读屏（多模态） | AC-A1, AC-B2 | Vision 理解测试（读图/读文档） | [ ] |
| R7 | "写" — 人格化文本生成 | AC-A1, AC-C2 | 人格化写作对比测试 | [ ] |
| R8 | "聊天" — Her 式陪伴对话，主动且温暖 | AC-A1, AC-C2, AC-D3 | 对话脚本盲测 + 主动关怀频率评估 | [ ] |
| R9 | "助理" — 工具调用与任务执行 | AC-D3 | MCP 工具 e2e 测试（日历/搜索/备忘录） | [ ] |
| R10 | "云服务器为主，手机 PC 手表多端访问" — 云中心 + 多端同步 | AC-A3, AC-B5, AC-C2, AC-D2 | 跨端一致性测试 + WatchOS 真机 + 离线补偿测试 | [ ] |
| R11 | "自进化能力类似于 hermes" — 越用越懂你，Skill 自生成与进化 | AC-B5, AC-D4 | GEPA 一轮进化 PR + 门闸通过证据 | [ ] |
| R12 | "基于 pi agent，只取核心，不重复造轮子" | AC-A2, KD-1/2 | Pi 核心清单 + Hermes 复用清单评审 | [ ] |
| R13 | 合规与伦理（隐含） | AC-A4, AC-D5 | AI 披露/自伤检测/危机路由/可删记忆 e2e | [ ] |
